...
if 'gunicorn' in sys.argv[0] or 'runserver' in sys.argv:
    try:
        ...
        if Complaint.objects.count() != len(all_coords):
            print(f"🔄 Seeding {len(all_coords)} complaints...")
            ...
            for i, (lat, lng) in enumerate(all_coords):
                Complaint.objects.create(
                    ...
                    latitude=lat,
                    longitude=lng,
                    description=f'Complaint #{i+1} in Attock',
                ...
                if (i + 1) % 10 == 0:
                    print(f"  ✅ Created {i+1}/{len(all_coords)} complaints")
            
            print(f"🎉 Database seeding complete! Created {len(all_coords)} complaints.")
        else:
