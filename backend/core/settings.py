...
if 'gunicorn' in sys.argv[0] or 'runserver' in sys.argv:
    try:
        ...
        # Check if database is empty or has no complaints
        if User.objects.count() == 0 or Complaint.objects.count() == 0:
            print("🌱 Seeding database on startup...")
            
            # Delete existing complaints to avoid duplicates
            ...
            # Create 17 complaints with your specific coordinates
            mehria_coords = [
            ...
            for i, (lat, lng) in enumerate(mehria_coords):
                ...
                print(f"  ✅ Created complaint #{i+1} at ({lat}, {lng})")
            
            print(f"🎉 Database seeding complete! Created {len(mehria_coords)} complaints.")
    except Exception as e:
        print(f"⚠️ Auto-seed error: {e}")

# ============================================
# AUTO-SEED DATABASE WITH 17 MEHRIA TOWN COMPLAINTS
# ============================================
import sys
if 'gunicorn' in sys.argv[0] or 'runserver' in sys.argv:
    try:
        ...
        if Complaint.objects.count() != 17:
            print("🔄 Seeding 17 Mehria Town complaints...")
            Complaint.objects.all().delete()
            
            users_data = [
                ('admin', 'admin123', True), ('citizen', 'citizen123', False),
            ...
            print(f"✅ Already have {Complaint.objects.count()} complaints.")
    except Exception as e:
        print(f"⚠️ Auto-seed error: {e}")
